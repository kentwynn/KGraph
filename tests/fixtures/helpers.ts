...
export async function runCli(repoPath: string, args: string[]): Promise<{ stdout: string; stderr: string; code: number }> {
...
export async function readJson<T>(repoPath: string, relativePath: string): Promise<T> {
  return JSON.parse(await readFile(path.join(repoPath, relativePath), "utf8")) as T;
}

export async function writeText(repoPath: string, relativePath: string, content: string): Promise<void> {
